    ...
    <div className="flex flex-col h-full gap-4 overflow-hidden">
      <div className="shrink-0 flex items-center justify-between">
        <div className="space-y-0.5">
          ...
          <p className="text-[10px] font-bold text-muted-foreground tracking-widest uppercase">关联校务数据 · 实时问答终端</p>
        </div>
        <Badge variant="outline" className="h-5 text-[9px] px-2 font-bold border-primary/20 text-primary bg-primary/5">服务就绪</Badge>
      </div>
      <AssistantContent />
    </div>
  );
...
function toPersistedMessages(messages: UIMessage[]): Array<{ role: "user" | "assistant"; content: string }> {
  return messages
...
function toUIMessages(rows: Array<{ id: string; role: "user" | "assistant"; content: string }>): UIMessage[] {
...
function AssistantContent() {
  ...
  const canSend = !!prompt.trim() && !!token && !!transport && status !== "submitted" && status !== "streaming";
    ...
    <div className="flex-1 min-h-0 flex overflow-hidden">
      ...
      <div className={cn(
        ...
        <div className="p-2.5 border-b shrink-0 flex items-center justify-between bg-muted/10">
          <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">对话历史</span>
          <Button 
            variant="ghost" 
            size="icon" 
            ...
            conversations.map(c => (
              <div 
                ...
                className={cn(
                  ...
                  activeConversationId === c.id ? "bg-muted/50 border-border/20 shadow-xs" : "hover:bg-muted/30"
                )}
              >
                <MessageSquareText className="h-3 w-3 shrink-0 opacity-40" />
                <span className="text-[11px] font-medium truncate flex-1 text-foreground/80">{c.title}</span>
                <button 
      ...
      <div className="flex-1 flex flex-col min-w-0 border border-border/50 rounded-xl bg-card/30 overflow-hidden transition-all duration-300">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-bold truncate max-w-50 text-foreground/80">
                {!activeConversationId ? "新对话 (草稿)" : (conversations.find(c => c.id === activeConversationId)?.title || "加载中...")}
              </span>
            </div>
          </div>
          <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground" onClick={() => setShowSettings(true)}>
          ...
          {messages.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-40">
               ...
               <div className="space-y-1">
                  ...
                  <p className="text-[10px] font-medium">输入任何问题，我将基于你的教务数据进行回答</p>
               </div>
            </div>
          )}
          {messages.map((m) => {
            ...
            return (
              <div key={m.id} className={cn("flex gap-3", isUser ? "flex-row-reverse" : "flex-row")}>
                 <div className={cn("h-8 w-8 rounded-lg flex items-center justify-center shrink-0 border", isUser ? "bg-primary border-primary shadow-sm" : "bg-muted border-border")}>
                   {isUser ? <UserIcon className="h-4 w-4 text-primary-foreground" /> : <Bot className="h-4 w-4 text-foreground" />}
                 </div>
                <div className={cn("max-w-[85%] space-y-1.5", isUser ? "items-end text-right" : "items-start text-left")}>
                  {isUser ? (
                    ...
                    <div className="text-[13px] leading-relaxed font-medium text-foreground/90 bg-muted/30 border border-border/40 p-4 rounded-xl shadow-xs">
                      <ReactMarkdown
                        ...
                        components={{
                          ...
                          code: ({ children }) => <code className="rounded bg-muted/60 px-1.5 py-0.5 font-mono text-[11px] font-bold">{children}</code>,
                        }}
                      >
                        {content}
                      </ReactMarkdown>
                  ...
                  {!isUser && tools.length > 0 && (
                    <div className="text-[9px] font-bold text-primary flex items-center gap-2 mt-1">
                      ...
                      <span className="tracking-widest">执行中: {tools.map(t => t.label).join(" & ")}</span>
                    </div>
                  )}
                </div>
              </div>
        ...
        <div className="shrink-0 border-t bg-muted/10 p-4">
          <div className="mx-auto flex w-full max-w-4xl items-end gap-2 rounded-xl border border-border/60 bg-background p-2 shadow-sm focus-within:border-primary/40 transition-all">
            ...
            />
            <Button size="icon" className="h-9 w-9 shrink-0 rounded-lg shadow-xs" disabled={!canSend} onClick={onSend}>
              {status === "submitted" || status === "streaming" ? <Loader2 className="h-4 w-4 animate-spin" /> : <SendHorizontal className="h-4 w-4" />}
            </Button>
          </div>
        </div>
      </div>
      ...
      <Dialog open={showSettings} onOpenChange={setShowSettings}>
        <DialogContent className="sm:max-w-112.5 border-border/60">
          <DialogHeader><DialogTitle className="text-base font-bold text-foreground">助手配置</DialogTitle></DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">接口地址</label>
              <Input className="font-mono text-xs h-9 bg-muted/20" value={settings.baseUrl} onChange={e => setSettings({...settings, baseUrl: e.target.value})} />
            </div>
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">模型名称</label>
              <Input className="font-mono text-xs h-9 bg-muted/20" value={settings.model} onChange={e => setSettings({...settings, model: e.target.value})} />
            </div>
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">密钥 (API Key)</label>
              <Input type="password" className="font-mono text-xs h-9 bg-muted/20" value={settings.apiKey} onChange={e => setSettings({...settings, apiKey: e.target.value})} />
            </div>
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">系统提示词</label>
              <Textarea className="text-xs min-h-25 bg-muted/20" value={settings.systemPrompt} onChange={e => setSettings({...settings, systemPrompt: e.target.value})} />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" size="sm" onClick={() => setShowSettings(false)}>取消</Button>
            <Button size="sm" onClick={onSaveSettings}>保存配置</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
