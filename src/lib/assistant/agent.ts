  ...
  return new ToolLoopAgent({
    ...
    tools: {
      get_user_info: tool({
        ...
        execute: async () => callMcpTool("get_user_info", { token }),
      }),
      get_personal_info: tool({
        description: "获取个人概览信息",
        inputSchema: z.object({}),
        execute: async () => callMcpTool("get_personal_info", { token }),
      }),
      get_grades: tool({
        description: "获取成绩信息",
        inputSchema: z.object({}),
        execute: async () => callMcpTool("get_grades", { token }),
      }),
      get_calendar: tool({
        description: "获取指定月份校历，格式如 2026年02月",
        inputSchema: z.object({ yearMonth: z.string() }),
        execute: async ({ yearMonth }) => callMcpTool("get_calendar", { token, yearMonth }),
      }),
    },
  });
}
